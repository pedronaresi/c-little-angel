...
//
//  target.c
//  Object code generator implementation for C- compiler.
//
...
void geraCodigoInstrucaoAtribuicao(Quadruple q) {
    ...
    if(q->op1.contents.variable.scope == NULL) {
        ...
        printCode(insertObjInst(createObjInst(_NOP, TYPE_J, NULL, NULL, NULL)));
...
void geraCodigoObjeto(Quadruple q) {
    ...
    while(q != NULL) {
        switch (q->instruction) {
            ...
            case HALT:
                printCode(insertObjInst(createObjInst(_RESET, TYPE_J, NULL, NULL, NULL)));
